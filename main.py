...
EMPLEADOS = {
    ...
    "edda_ramos": {"nombre": "Edda Ramos Pérez", "empresa": "GRUPO ICADEPRO", "tipo_acceso": "oficina"}
  }
...
def rectificar_jornada(id: int, entrada: str, salida: str, motivo: str):
    ...
    # 🔹 OBTENER DATOS ACTUALES DEL FICHAJE
    cursor.execute("""
        SELECT hora_entrada, hora_salida, observaciones
        FROM fichajes
        WHERE id = ?
    """, (id,))
    ...
    if observaciones_actuales and observaciones_actuales.strip():
        observaciones = (
            observaciones_actuales.strip()
            + " | "
            + nueva_observacion
        )
    else:
